...
import clockplucker.data.TextValue
...
class RoleSolverTest {
    ...
    fun testRoleSolverMaximumSatisfactionWithSurprise() = runBlocking {
        ...
        assignments.forEach { (player, assignment) ->
            assert(assignment.first in player.selectedChars || assignment.second in player.selectedChars)
        }
    }
    ...
    fun testRoleSolverSurprise() = runBlocking {
        ...
        assert(assignments.values.any { it.first.id == "drunk" })
    }
    ...
    fun testRoleSolverAtheist() = runBlocking {
        // 1. Create script with 11 characters
        val characters = mutableListOf<Character>()
        val atheist = Character(
            "atheist",
            TextValue.Raw("Atheist"),
            type = CharType.TOWNSFOLK,
            icon = 0,
            ability = TextValue.Raw(""),
            overrideModifiers = listOf(
                CharType.MINION,
                CharType.DEMON
            )
        )
    ...
    fun testRoleSolverDependency() = runBlocking {
        ...
        val t2 = Character(
            "t2",
            TextValue.Raw("Townsfolk 2"),
            type = CharType.TOWNSFOLK,
            icon = 0,
            ability = TextValue.Raw(""),
            dependsOn = "t1"
        )
        val otherChars = (3..6).map { i ->
            Character(
                "t$i",
                TextValue.Raw("Townsfolk $i"),
                type = CharType.TOWNSFOLK,
    ...
    fun testRoleSolverHeretic() = runBlocking {
        ...
        val lleech = Character(
            "lleech",
            TextValue.Raw("Lleech"),
            type = CharType.DEMON,
            icon = 0,
            ability = TextValue.Raw("")
        )
        val otherDemon = Character(
            "imp",
            TextValue.Raw("Imp"),
            type = CharType.DEMON,
            icon = 0,
    ...
    fun testRoleSolverHermitDrunk() = runBlocking {
        val drunk = Character(
            ...
            type = CharType.OUTSIDER,
            icon = 0,
            ability = TextValue.Raw("thinks they are townsfolk"),
            thinksTheyAre = listOf(CharType.TOWNSFOLK)
        )
        val demon = Character(
            id = "demon",
        ...
        val containsDrunk = true
        val containsLunatic = false
        val hermitThinksTheyAre = when {
            containsDrunk && containsLunatic -> listOf(CharType.DEMON)
            containsDrunk -> listOf(CharType.TOWNSFOLK)
            containsLunatic -> listOf(CharType.DEMON)
            else -> listOf()
        }
        val hermit = Character(
            id = "hermit",
            ...
            type = CharType.OUTSIDER,
            icon = 0,
            ability = TextValue.Raw("has all outsider abilities"),
            thinksTheyAre = hermitThinksTheyAre
        )

        val characters = listOf(hermit, drunk, demon) +
                (1..6).map { i ->
                    Character(
                        "t$i",
                        TextValue.Raw("Townsfolk $i"),
        ...
        val solver = RoleSolver(
            ...
            availableChars = characters,
            baseCount = baseCount,
            surpriseChances = mapOf(hermit to 1f)
        )
        val assignments = solver.optimizeAssignments()

        assert(assignments.isNotEmpty())

        // the assigned hermit should think they are a townsfolk
        assert(assignments.filter { it.value.first.id == "hermit" && it.value.second?.type == CharType.TOWNSFOLK }.size == 1)
    }

    @Test
    fun testRoleSolverHermitLunatic() = runBlocking {
        val lunatic = Character(
            id = "lunatic",
            name = TextValue.Raw("Lunatic"),
            type = CharType.OUTSIDER,
            icon = 0,
            ability = TextValue.Raw("thinks they are a Demon"),
        ...
        val containsDrunk = false
        val containsLunatic = true
        val hermitThinksTheyAre = when {
            containsDrunk && containsLunatic -> listOf(CharType.DEMON)
            containsDrunk -> listOf(CharType.TOWNSFOLK)
            containsLunatic -> listOf(CharType.DEMON)
        ...
        val characters = listOf(hermit, lunatic, demon) +
                (1..6).map { i ->
                    Character(
                        "t$i",
                        TextValue.Raw("Townsfolk $i"),
    ...
    @Test
    fun testRoleSolverHermitDrunkLunatic() = runBlocking {
        val lunatic = Character(
            id = "lunatic",
            name = TextValue.Raw("Lunatic"),
            type = CharType.OUTSIDER,
        ...
        val characters = listOf(hermit, lunatic, drunk, demon) +
                (1..6).map { i ->
                    Character(
                        "t$i",
                        TextValue.Raw("Townsfolk $i"),
